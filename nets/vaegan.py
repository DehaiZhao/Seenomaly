...
def vaegan_arg_scope(is_training = True, weight_decay=0.0005):
...
def discriminator(inputs, is_training = True):
  with tf.variable_scope('discriminator') as sc:
    ...
    with slim.arg_scope([slim.conv3d], kernel_size = 4, padding = 'same', activation_fn = tf.nn.leaky_relu, outputs_collections=end_points_collection):
      ...
      print (net.shape) 
      net = slim.conv2d(net, 1, [1, 1], activation_fn = None, scope='fc7')
      net = tf.squeeze(net)   
      # Convert end_points_collection into a end_point dict.
      end_points = slim.utils.convert_collection_to_dict(end_points_collection)
      print (net.shape) 
...
def vaegan(inputs, label, num_classes=50, is_training=True):
  with tf.variable_scope('vaegan', reuse=tf.AUTO_REUSE) as sc:
    if is_training:
      z, x_, end_points = generator(inputs, is_training = is_training)
      label_fake, _ = discriminator(x_, is_training = is_training)
      label_real, end_points_d = discriminator(inputs, is_training = is_training)
      end_points.update(end_points_d)

      context_loss = tf.losses.absolute_difference(inputs, x_)
