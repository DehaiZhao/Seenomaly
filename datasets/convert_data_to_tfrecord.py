...
def _image_to_tfexample(image_data, image_format, height, width, class_id, box):
...
def _get_filenames_and_classes():
  with open(os.path.join(dataset_dir, 'label.txt')) as list_file:
...
def _convert_dataset(split_name, filenames):
  ...
  for filename in filenames:
    img_files.append(filename.strip().split()[0])
    class_names.append(filename.strip().split()[1])
    boxes.append(filename.strip().split()[2])
      ...
      for shard_id in range(_NUM_SHARDS):
        ...
        with tf.python_io.TFRecordWriter(output_filename) as tfrecord_writer:
          ...
          for i in range(start_ndx, end_ndx):
            ...
            height, width = image_reader.read_image_dims(sess, image_data[0])
            class_id = int(class_names[i])
            box = [int(value) for value in boxes[i].split(',')]
            example = _image_to_tfexample(image_data, b'jpg', height, width, class_id, box)
